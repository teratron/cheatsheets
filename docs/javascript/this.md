...
// Для функций, определенных в глобальном контексте, - это объект Window, который представляет окно браузера.
function foo() {
    var bar = "bar2";
...
контексте
определяется
переменная.// Например, возьмем следующий код:
    var
school = {
    title: "Oxford", courses: ["JavaScript", "TypeScript", "Java", "Go"], printCourses: function () {
